...
window.doRegister = function() {
    ...
    if (users.find(u => u.email === email)) { showToast('Email уже используется', 'error'); return; }
...
function recordGameHistory(game) {
    if (!currentUser) return;
    const key = 'igroteka_history_' + currentUser.username;
    const history = load(key, []);
    // Не дублируем запись если та же игра уже есть в последних 5 минутах
    const last = history[0];
    if (last && last.gameId === String(game.id) && Date.now() - new Date(last.playedAt).getTime() < 5 * 60 * 1000) return;
    history.unshift({
    ...
    // Хранить не более 50 записей
    saveToStorage(key, history.slice(0, 50));
}

function getGameHistory() {
    if (!currentUser) return [];
    return load('igroteka_history_' + currentUser.username, []);
}
...
window.openProfileModal = function(tab) {
    ...
    renderProfileModal(tab || 'profile');
...
function renderProfileModal(activeTab) {
    const achieved = getUserAchievements();
    const xp = currentUser.xp || 0;
    ...
    const headerHtml = `
        <div class="profile-header">
            ...
            <div class="profile-header-info">
                ...
                ${fullUser.email ? `<div class="profile-email"><i class="fas fa-envelope"></i> ${escHtml(fullUser.email)}</div>` : ''}
                <div class="profile-xp">Уровень ${lvl} · <span>${xp} XP</span></div>
                <div style="margin-top:6px"><div class="xp-track" style="width:200px"><div class="xp-fill" style="width:${pct}%"></div></div></div>
            </div>
        </div>`;
    ...
    const tabs = [
        ...
        { id: 'achievements', icon: 'fa-trophy', label: `Достижения (${achieved.length}/${ACHIEVEMENTS.length})` }
    ];
    const tabsHtml = `<div class="profile-tabs">${tabs.map(t =>
        `<button class="profile-tab ${activeTab === t.id ? 'active' : ''}" onclick="switchProfileTab('${t.id}')">
    ...
    if (activeTab === 'profile') {
        const joinDate = fullUser.joinedAt ? formatDate(fullUser.joinedAt) : '—';
        const gamesPlayed = new Set(history.map(h => h.gameId)).size;
        contentHtml = `
            <div class="profile-stats-grid">
                <div class="profile-stat"><div class="stat-value">${lvl}</div><div class="stat-label">Уровень</div></div>
                <div class="profile-stat"><div class="stat-value">${xp}</div><div class="stat-label">XP</div></div>
                <div class="profile-stat"><div class="stat-value">${gamesPlayed}</div><div class="stat-label">Игр открыто</div></div>
                <div class="profile-stat"><div class="stat-value">${achieved.length}</div><div class="stat-label">Достижений</div></div>
            </div>
            <div class="profile-info-block">
                <div class="profile-info-row"><span><i class="fas fa-user"></i> Логин</span><span>${escHtml(currentUser.username)}</span></div>
                ${fullUser.email ? `<div class="profile-info-row"><span><i class="fas fa-envelope"></i> Email</span><span>${escHtml(fullUser.email)}</span></div>` : ''}
                <div class="profile-info-row"><span><i class="fas fa-calendar"></i> Дата регистрации</span><span>${joinDate}</span></div>
                <div class="profile-info-row"><span><i class="fas fa-shield-alt"></i> Роль</span><span>${ROLES[currentUser.role]||'Пользователь'}</span></div>
            </div>`;
    }

    if (activeTab === 'history') {
        if (history.length === 0) {
            contentHtml = `<div class="profile-empty"><i class="fas fa-history"></i><p>История пуста — откройте любую игру!</p></div>`;
        } else {
            const all = getAllGames();
            contentHtml = `<div class="history-list">` + history.map(h => {
                const cat = categories.find(c => c.id === h.categoryId);
                return `<div class="history-item">
                    <div class="history-icon">${h.gameIcon}</div>
                    <div class="history-info">
                        <div class="history-name">${escHtml(h.gameName)}</div>
                        <div class="history-meta">${escHtml(cat?.name || 'Без категории')} · ${formatDate(h.playedAt)}</div>
                    </div>
                    <button class="btn-primary btn-sm" onclick="closeModal('profileModal'); openGameModal('${h.gameId}')">
                        <i class="fas fa-play"></i>
                    </button>
                </div>`;
            }).join('') + `</div>
            <div style="text-align:center;margin-top:12px">
                <button class="btn-secondary btn-sm" onclick="clearGameHistory()"><i class="fas fa-trash"></i> Очистить историю</button>
            </div>`;
        }
    }
    ...
    if (activeTab === 'achievements') {
        ...
        contentHtml = `<div class="achievements-grid">${achHtml}</div>`;
    }

    document.getElementById('profileModalContent').innerHTML = headerHtml + tabsHtml + `<div class="profile-tab-content">${contentHtml}</div>`;
}

window.clearGameHistory = async function() {
    if (!await showConfirm('Очистить всю историю игр?')) return;
    saveToStorage('igroteka_history_' + currentUser.username, []);
    renderProfileModal('history');
};
...
function renderAdminUsers(content) {
        ...
        <tr><td colspan="7" style="padding:0">
            <div class="user-edit-form" id="ue_${u.username}">
                <div class="form-row">
                    ...
                    <div class="form-group"><label>Email</label><input type="email" id="ue_email_${u.username}" value="${escHtml(u.email||'')}"></div>
                </div>
                <div class="form-row">
    ...
    content.innerHTML = `
        ...
        <table class="admin-table"><thead><tr><th>Имя</th><th>Логин</th><th>Email</th><th>Роль</th><th>XP</th><th>Зарегистрирован</th><th>Действия</th></tr></thead><tbody>${rows||'<tr><td colspan="7" style="text-align:center">Нет пользователей</td></tr>'}</tbody></table>`;
}
...
window.adminSaveUser = function(username) {
    ...
    users[idx].email = document.getElementById('ue_email_' + username)?.value.trim() || users[idx].email;
